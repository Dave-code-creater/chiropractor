const PreliminaryService = require('../services/preliminary.service');
const {
    CREATED,
    OK,
} = require('../utils/httpResponses');
class PreliminaryController {
    static async create(req, res) {
        const result = await PreliminaryService.create(req.body, req);
        return new CREATED({ metadata: result }).send(res);
    }

    static async list(req, res) {
        const result = await PreliminaryService.list(req);
        return new OK({ metadata: result }).send(res);
    }

    static async update(req, res) {
        const result = await PreliminaryService.update(req, req.body);
        return new OK({ metadata: result }).send(res);
    }

    static async getByID(req, res) {
        const result = await PreliminaryService.getById(req);
        return new OK({ metadata: result }).send(res);
    }

    static async delete(req, res) {
        const result = await PreliminaryService.delete(req);
        return new OK({ metadata: result }).send(res);
    }
}
module.exports = PreliminaryController;
