...
const { userCreateValidator, patientUpdateSchema, clinicalNotesSchema, vitalsSchema } = require('../validators');
...
class UserService {
  ...
  static async createPatient(patientData, req) {
    const { error } = userCreateValidator.validate(patientData);
  ...
  static async getVitalsHistory(patientId) {
    try {
      ...
      const vitals = await patientRepo.getVitalsHistory(patientId);
      ...
      return vitals.map(vital => ({
        ...
        bmi: vital.height && vital.weight ? 
          (vital.weight / Math.pow(vital.height / 100, 2)).toFixed(1) : null,
        notes: vital.notes,
        recorded_by: vital.recorded_by,
        doctor_name: vital.doctor_name,
      ...
      throw new InternalServerError('Failed to retrieve vitals history', '5008');
    }
  }
}
